...
//        Group objects by a specific field (e.g., age)
        ...
        System.out.println("grouped by age " + pmap);
...
//        Identify duplicate elements in a list
        ...
        List<Integer> duplicatelist = Arrays.asList(1, 2, 3, 2, 4, 5, 3, 6);

        Map<Integer, Long> duplicatemap = duplicatelist.stream().collect(Collectors.groupingBy(
                i -> i,
                HashMap::new,
                Collectors.counting()
        ...
        List<Integer> duplicatemapans = duplicatelist.stream().collect(Collectors.groupingBy(
                i -> i,
                LinkedHashMap::new,
                Collectors.counting()
        )).entrySet().stream().filter(e -> e.getValue() > 1).map(e -> e.getKey()).toList();
        System.out.println(duplicatemapans);
